...
class Configurazione
{
    ...
    public static function ottieniConfigurazione()
    {
        ...
        $conf = new \stdClass;
        $conf->{"corso-di-laurea-default"} = JSON::leggiJSON(PLUGIN_BASE_PATH . self::PATH_DEFAULT);

        // carica la configurazione dei corsi
        $conf->{"corsi-di-laurea"} = [];
        foreach (glob(PLUGIN_BASE_PATH . self::PATH_CORSI . "/*.json") as $file) {
    ...
    {
        // carica schema
        $schema = JSON::leggiJSON(PLUGIN_BASE_PATH . self::PATH_SCHEMA);
    ...
    { 
        ...
        array_map("unlink", glob(PLUGIN_BASE_PATH . self::PATH_CORSI . "/*.json"));
